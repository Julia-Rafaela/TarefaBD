...
SELECT c.marca,
	   c.modelo,
	   SUM(v.distanciaPercorrida) as SomaTotal
FROM carro c, viagem v
WHERE c.id = v.idCarro
GROUP BY c.marca, c.modelo, v.distanciaPercorrida, v.data
...
SELECT e.nome,
SUM(v.distanciaPercorrida) AS Total_distanciaPercorrida,
FORMAT(AVG(v.distanciaPercorrida), 'N2') AS mediaDist
FROM empresa e, viagem v, carro c
WHERE e.id = c.idEmpresa
AND c.id = v.idCarro
GROUP BY e.nome
...
SELECT e.nome,
MAX(v.distanciaPercorrida) AS Maior_distanciaPercorrida
FROM empresa e, viagem v, carro c
WHERE e.id = c.idEmpresa
AND c.id = v.idCarro
GROUP BY e.nome
...
SELECT e.nome,
      COUNT(c.modelo) AS qtd_carros
FROM empresa e, carro c
WHERE e.id = c.idEmpresa
GROUP BY e.nome
HAVING COUNT(c.modelo) > = 3
ORDER BY  COUNT(c.modelo)
...
SELECT COUNT(v.data) AS quantidade,
       c.marca, 
       c.modelo,
	   e.nome
FROM viagem v, carro c, empresa e
WHERE v.idCarro = c.id
AND e.id = c.idEmpresa 
...
SELECT e.nome,
	   ...
	   CASE  
	   WHEN v.distanciaPercorrida <=  1000 THEN SUM(v.distanciaPercorrida * 10.00)
	   WHEN v.distanciaPercorrida > 1000 THEN SUM(v.distanciaPercorrida * 15.00)
	   END 'valor total'    
